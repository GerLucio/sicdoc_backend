<?PHP
	...
	$revision->id_revision = trim($revision_request['revision']['id_revision']);
	...
	$consulta_inserta = "update revisiones set
	...
	oci_bind_by_name($ejecuta_consulta_inserta, ':id_revision', $revision->id_revision);
	...
	if($alta_correcta){
		...
		try {
			...
			//Asunto del correo 
			$mail->Subject = utf8_decode("Modificaciones en solicitud de cambio de revisión de un documento"); 
			$mail->isHTML(true);
			  $mail->Body = utf8_decode("<meta http-equiv='Content-Type' content='text/html; charset=utf-8'/> 
				<p>
				El proceso <strong>".$proceso->nombre."</strong> modificó la solicitud de cambio de revisión del 
				documento <strong>".$documento->codigo." ".$documento->nombre."</strong>.
				</p><br><br>
