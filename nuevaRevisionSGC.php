<?PHP
	...
	if($correcto){
		...
		$consulta_no_revision = "select no_revision -1 as no_revision from v_revisiones_vigentes where id_documento = :id_documento";
		...
		try {
			...
			if($row != false){			
				...
				$ejecuta_consulta_usuarios = oci_parse($conexion, $consulta_usuarios);
				oci_bind_by_name($ejecuta_consulta_usuarios, ':proceso', $proceso->id_proceso);
				oci_execute($ejecuta_consulta_usuarios);
				while($row = oci_fetch_array($ejecuta_consulta_usuarios, OCI_ASSOC+OCI_RETURN_NULLS)){
					$usuarios[] = $row;
				}
